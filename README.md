...
use dbdiagram to generate sql query.
use migrate to create new up and down migration and write your up and down sql query to your database.
`Note`: You should write drop statements for the last query first

## -- Lecture 16: Handle Error Database --

You should use http.Forbidden code for request which is not allowed for the database constaints,
it's default ServerInternalError.
To get the error, use
	if err != nil {
  ...
  use log.Println(pqErr.Code.Name()) to know the case violation

## -- Lecture 17: Hash password with Bctypt --

`Hash Password Components:`

$2a$10$6mG.vIFyvNsjTYQRnz/4ou4IC1aL8ZVBMLg9MfmF.g2SGNxlwWn1K
 - Hash function identifier: `$2a$10$`
 - Number of Interations (COST): `10`
 - Salt value: `6mG.vIFyvNsjTYQRnz/4ou` 22 characters
 - Hash password: `4IC1aL8ZVBMLg9MfmF.g2SGNxlwWn1K` 24 characters

`Process:`

When you register:  Password + Hash Function(Cost, Salt) -> NewHashPassword -> Save to database
When you login:     Password + Hash Function(Cost, Salt) -> YourHashPassword
                    DB -> DBHashPassword
                    If YourHashPassword == DBHashPassword -> Correct password

Password hashing functions are designed to be one-way functions, 
Meaning that not be able to decode or recover the original password.
Number of Interations higher -> stronger password.
